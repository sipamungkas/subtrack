...
export function useSubscriptionStats() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ data: SubscriptionStats }>(
...
export function useUpdateSubscription() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data } = await api.put<{ data: Subscription }>(
        `/api/subscriptions/${id}`,
        input
      );
      return data.data;
...
export function useTestSubscriptionNotification() {
  return useMutation({
    mutationFn: async (id: string) => {
      await api.post(`/api/subscriptions/${id}/test-notification`);
    },
    onError: (error: any) => {
      // Error handling is managed by the component via onError/onSuccess callbacks or try/catch
